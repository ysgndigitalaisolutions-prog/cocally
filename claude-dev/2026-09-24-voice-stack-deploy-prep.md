...
- Deploy (VM) failed twice at "Copy stack files": /opt/cocally is root-owned, deployer's OS Login user cannot write. Workflow now stages in ~ and `sudo install`s. Missing GitHub vars VM_NAME/GCP_ZONE/APP_DOMAIN added; `deploy/gcp/git_variables.txt` is the reference (setup.sh rewrites it).
- Twilio SIP credential `cocallyprod` created (CR2021253a4b91f51e34ddffee24a72690) on `cocally-livekit-creds`. LiveKit prod outbound trunk `ST_Dy4tHDsvd6Y8` created from the laptop (api, not the VM seed). DNS `app.co-cally.com` → 35.244.108.69 live.
- `deploy/gcp/.env` now: DOMAIN=app.co-cally.com, TELEPHONY_DRIVER=SIP, LIVEKIT_SIP_TRUNK_ID set. Needs: setup.sh re-run (Nithin), GitHub vars APP_DOMAIN + TELEPHONY_DRIVER updated, full re-deploy, then provision-tenant.sh.
