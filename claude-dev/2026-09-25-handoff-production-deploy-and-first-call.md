...
## 13. First AI call result + AU carrier details (2026-09-25 ~09:30 IST)

- AI call `6ab5f12d7ac6e6ed84a84c08` connected and Sam spoke. Worker log: Groq **HTTP 429** on `qwen/qwen3.8-27b` — on_demand tier is **8 000 input tokens/min per model** (read from `x-ratelimit-limit-tokens`); a call burns that in ~3 turns. Groq developer-tier upgrade is "temporarily unavailable". Nithin declined Anthropic; another provider TBD (Cerebras/OpenAI/Gemini all plug into the existing `openai.LLM(base_url=…)` path). `agent.py` now wraps the LLM in `FallbackAdapter([qwen, gpt-oss-120b])` (separate buckets).
- Also in that log, unexplained: `scripted disclosure failed ()` = 20 s timeout awaiting playout of the pre-rendered disclosure; first two turns `transcription_delay` 14.5 s / 23 s then 0.4 s; `tts_ttfb=0.88s` cold. Need the raw worker log for that call + `uptime`/`docker stats` (CPU starvation on e2-standard-2 suspected).
- **AU carrier (Telvoq)**: `lon1.telvoq.com:5060` UDP, tech prefix `1701`, DID `61238196956` (+61 2 3819 6956), dial format `1701<CC><number>`, **IP allow-list auth** — they want our source IPs. LiveKit Cloud publishes static IP ranges only for Canada/EU/India/Japan/US (`143.223.88.0/21`, `161.115.160.0/19`, `153.57.128.0/18`); **Australia is not covered**. Options: (1) ask Telvoq for digest auth (username/password) — preferred, everything is built for it; (2) if IP-only, pin SIP to a static-IP region (India `ind` is nearest to Sydney among the covered ones) and give them those three ranges — untested, adds RTT; (3) self-host a SIP gateway with a fixed IP — out of scope for the pilot.
- Added `SIP_DIAL_PREFIX` (config → `livekit.service.ts dialOut`; setup.sh; env files). Blank for Twilio; `1701` for Telvoq. tsc clean, uncommitted.
- Telvoq insists on IP allow-list. **Plan:** outbound trunk `destinationCountry: 'IN'` (LiveKit outbound region pinning: "calls originate from a server within the specified country"; India has published static ranges). Give Telvoq `143.223.88.0/21`, `161.115.160.0/19`, `153.57.128.0/18`. Caveat: if the India region is down LiveKit fails over to a nearby region whose IPs are not whitelisted → those calls fail auth. `provision-sip-trunk.ts` now takes `--destination-country` / `SIP_DESTINATION_COUNTRY` and makes username/password optional; setup.sh + env files carry `SIP_DESTINATION_COUNTRY` (default AU). tsc clean, uncommitted. Telvoq's POP is `lon1` (London): ask them for an APAC/AU POP, otherwise every AU call hairpins through London (~250 ms+ each way on media).

## 14. Cerebras is the on-call LLM (2026-09-25)

- Groq developer-tier upgrade unavailable; Anthropic declined. Nithin supplied a **Cerebras** key (USD 25 credit). Measured: `qwen-3.8-27b` 150k tokens/min, `gpt-oss-120b` 500k/min; ~370 ms first token from India (cold connection). `reasoning_effort: "none"` verified to stop qwen's hidden thinking (without it, content is null and tokens are burnt).
- **Worker** (`agent.py`): `_PROVIDERS` table (cerebras, groq) → `FallbackAdapter` chain `cerebras:qwen → cerebras:gpt-oss → groq:qwen → groq:gpt-oss` (only providers with a key). `LLM_PROVIDER` picks primary (defaults to cerebras when `CEREBRAS_API_KEY` is set); `LLM_MODEL` picks the family. `_reasoning_effort` matches by family. Boot check needs one of `CEREBRAS_API_KEY|GROQ_API_KEY`. Local `agent.py start` clean.
- **API**: `CerebrasLlm` adapter (`http.adapters.ts`), first in `pal.service` LLM order; `CEREBRAS_API_KEY` in config/vault. tsc clean, 19/19 tests, adapter verified end to end in JSON mode.
- Key stored in `deploy/gcp/.env`, `agent-worker/.env`, `apps/api/.env`; `setup.sh` passes `CEREBRAS_API_KEY` + `LLM_PROVIDER`. **Key was pasted in chat — rotate before the pilot** (same as ElevenLabs).
- Next: push (worker + api rebuild), `./deploy/gcp/setup.sh` (new env vars must reach the VM **before** the deploy restarts the containers, or run it and then `up -d api worker`), then an AI dial.

## 15. UI phase 1 (separate session, same day) is in the same working tree

See `2026-09-25-ui-simplification-phase1.md`: role-based grouped sidebar (`lib/nav.ts`, `components/Sidebar.tsx`), `lib/roles.ts` with `homeFor` landing rule, route guard in `(app)/layout.tsx`, workspace floor/roster only for supervisor roles, first web unit tests (vitest, 14 cases) wired into CI. OWNER is deliberately not a desk role: the "Takes calls" toggle (§12) adds AGENT and with it My desk. Combined tree verified 2026-09-25: shared build, web typecheck, web tests 14/14, web production build (21 routes), api tsc + 19/19. Everything from §4 onward is still uncommitted and ships in one push.

## 16. Telvoq whitelisted; first probes → 403 (2026-09-25 05:10–05:17 UTC)

Trunk `ST_fGuwwzkowYPr` created in prod LiveKit (address `lon1.telvoq.com:5060`, UDP, number `+61238196956`, no auth, `destination_country=IN`). Four raw `CreateSIPParticipant` probes from the laptop (no app): India number with `+61…` caller ID, without plus, trunk-default caller ID, and the DID itself as destination → **all `403 Forbidden (permission_denied)` within 1.3–3.8 s**. Fast reject = ACL most likely: either LiveKit did not originate from the India static ranges, or Telvoq's whitelist is not applied/complete. Asked Telvoq for the source IP they saw on our INVITEs at that time. Twilio trunk remains active in `.env`; Telvoq id noted there as a comment.
- **Source-IP probe (05:25 UTC):** temp trunk → our VM with a UDP listener on 5060. LiveKit's INVITE arrived from **143.223.94.27** (inside 143.223.88.0/21) with `destination_country=IN` — region pinning to a static-IP range is confirmed working. Telvoq's 403 is therefore on their side (ACL not applied, or destination/prefix policy). Probe trunk deleted; firewall rule `cocally-sip-probe` must be deleted by Nithin.
- Prefix changed by Telvoq to **45**; retest 05:39–05:40 UTC → still `403 Forbidden` in ~1 s (all variants). Telvoq: "no hits in CDR/switch logs" and "source IP totally different from whitelisted range" (no IP given). `lon1.telvoq.com` = 23.229.7.251. Raw SIP from the laptop (unknown IP): OPTIONS → `404 Not Found`, INVITE → **`401 Account Suspended`** in 0.16 s. LiveKit's calls get **403 Forbidden** — a different code, so their edge (SBC) does classify our source differently from an unknown one; the switch behind it never sees the call. Asked Telvoq for the exact source IP and the edge/SBC log line, not the switch CDR.
- **06:07 UTC, after Telvoq "tried once more":** 403 gone. India number → `480 Temporarily Unavailable`; own DID → `486 Busy Here` (expected: no inbound trunk). Edge ACL now passes; remaining issue is routing/permission for 91 destinations or dial format (prefix now 45). Telvoq had whitelisted the single IP 143.223.94.27; ranges still requested. Probe trunk `ST_YBfe4S4mtm5y` exists (delete when done).
- Telvoq: routing is **AU-only** on this account ("pls dial australia"); without the 45 prefix the edge demands auth (prefix = account identifier). Their AU test number **+61 2 8018 5636**: 06:10:49 UTC → no final response in 30 s; 06:11:39 UTC → none in 75 s (`408 sip request timed out`). No 4xx this time, so the INVITE is accepted and either rings out unanswered or gets no provisional response. Awaiting Telvoq's view (did it ring / who answers that line).
- 06:33 UTC to +61 2 8018 5636: **answered after ~18 s, then `412 common audio codec not found`**. The call now completes end to end at the SIP level; Telvoq's answer SDP offers no codec LiveKit supports (LiveKit: PCMU/PCMA G.711, G.722, Opus). Ask Telvoq to enable G.711 A-law/µ-law on our route/account (likely G.729-only today). No codec setting exists on the LiveKit trunk side.
- Telvoq says G.711/G.722/G.729 all enabled with transcoding and cannot find the 06:33 call in CDR (again: edge handles it, switch never logs). 06:46:25 UTC retry → rang ~70 s then `486 Busy Here`. Results on their test line vary (timeout / answered-then-codec-fail / busy), consistent with a real attended phone. LiveKit gives no SIP Call-ID for failed dials; to hand Telvoq a Call-ID + our SDP offer, capture a full INVITE at the VM listener via probe trunk `ST_YBfe4S4mtm5y`.
- **06:52:11 UTC: Telvoq works end to end.** Call to +61 408 988 859 (dialled `4561408988859`, caller ID 61238196956) answered in 3.3 s, LiveKit call `SCL_JzhUWprmXif5`, audio track up for the 12 s we held it. Their own test line 61 2 8018 5636 was simply unattended. Probe trunk deleted; LiveKit trunks now: Twilio `ST_Dy4tHDsvd6Y8` (active), Telvoq `ST_fGuwwzkowYPr`. **Switch to Telvoq** = `LIVEKIT_SIP_TRUNK_ID=ST_fGuwwzkowYPr`, `SIP_DIAL_PREFIX=45`, `SIP_TRUNK_ADDRESS=lon1.telvoq.com:5060`, `SIP_TRUNK_NUMBERS=+61238196956`, `SIP_DESTINATION_COUNTRY=IN`, then setup.sh + `up -d api worker`. Still open with Telvoq: CIDR ranges (only 143.223.94.27 whitelisted → other LiveKit addresses will 403), APAC POP, DID inbound routing. Do the switch AFTER the Twilio AI test proves the conversation; Telvoq is AU-only so the +91 test phones cannot be used through it. Nithin must delete firewall rule `cocally-sip-probe` if it was recreated.
